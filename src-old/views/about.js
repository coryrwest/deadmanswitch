export default () => (
  <div>
    ...
    <p><strong>Check Interval</strong>: How often you want DeadManSwitch to send you a check. (ex. every 24 hours, every week, etc.) Can be any time period.</p>
    <p><strong>Check Time Range</strong>: Time range that you want checks to happen in. This is used for very short Check Intervals to prevent a trigger while you are sleeping.</p>
    <p><strong>Check Method</strong>: Email, SMS, or both. If both DeadManSwitch will accept an alive ping from either method.</p>
    <p><strong>Trigger Interval</strong>: How long after the last missed check you want to add a trigger to the counter. (ex. trigger the counter 2 hours after missed check.)</p>
    <p><strong>Trigger Count</strong>: How many triggers before the switch is triggered.</p>
    <p><strong>Trigger Multiplier</strong>: Use this to increase the Trigger Interval after every increase of the counter. The scale is exponential. 
    (ex. interval is set to every 2 hours and multipler is 2, then next count won't be added until 4 hours after the last interval, then next 8 hours, etc.)</p>
    <p><strong>Vacation Delay</strong>: How long you want to turn DeadManSwitch off for when you are away. 
    If you are going on vacation for two weeks and won't be able to respond to pings then set this.</p>
    <p><strong>Email Addess</strong>: Email to send message to.</p>
    <p><strong>Phone Number</strong>: Phone number to send SMS to.</p>
    <p><strong>Subject</strong>: Subject of the message (email only).</p>
